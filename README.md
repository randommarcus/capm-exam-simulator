...
- **Question navigator** — a ☰ toggle button in the quiz header opens a scrollable grid of every question in the session. Each pill shows its number plus a status indicator: a green **✓** for correct, a red **✗** for wrong, plain for unanswered. These are CSS-styled unicode text characters (not emoji) for consistent rendering across all platforms. Clicking any pill jumps directly to that question. The navigator is accessible at all times since the header is sticky.
